	...
	<?php if ($rs['cover']) : ?>
		<style>
			.pt-restaurantspage .pt-restaurant-head:before {
				background-image: url(<?= path . '/' . $rs['cover'] ?>)
			}
		</style>
	<?php endif; ?>
	<div class="container">
					...
					?>
							<div class="col-4">
								<div class="pt-item">
									...
									<div class="pt-title"><a href="<?= path ?>/restaurants.php?id=<?= $rs['id'] ?>&t=<?= fh_seoURL($rs['name']) ?>">
											<h3><?= $rs['name'] ?></h3>
										</a></div>
									<div class="pt-stars"><?php echo fh_stars($rs['id'], "restaurant") ?></div>
